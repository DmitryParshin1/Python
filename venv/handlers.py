...
async def mes_set(message: types.Message):
    ...
    if not new_game:
        if count.isdigit():
            ...
            await message.answer(f'{message.from_user.first_name}, напиши цифрам')
    else:
        await message.answer(f'{message.from_user.first_name}, во время игры, нельзя менять правила!')
...
async def mes_all(message: types.Message):
    ...
    if new_game:
        if message.text.isdigit():
            ...
            if total <= 0:
                await message.answer(f'{message.from_user.first_name} ты победил!')
                new_game = False
                total = 150
            else:
